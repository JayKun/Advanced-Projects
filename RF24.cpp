...
/****************************************************************************/
uint8_t RF24::setAddress(const uint8_t* buf, uint8_t len)
{
	return write_register(RX_ADDR_P0, buf, len);
}
...
uint8_t RF24::rxMode()
{
	...
	write_register(EN_RXADDR, 0x01);	// Enable first data pipe
	write_register(SETUP_AW, 0x03);	// 5 bytes address
	write_register(SETUP_RETR, 0xFF);	// 15 retransmit, 4000us pause
	write_register(RF_CH, 0x00);	// channel 8
	setDataRate(RF24_250KBPS);
	write_register(NRF_STATUS, 0x70);		// Clear status register
	write_register(RX_PW_P0, 0x0A);	// RX payload of 10 bytes
...
bool RF24::setDataRate(rf24_datarate_e speed)
{
	...
	//different chips require different waiting times (dwbi)
...
void RF24::setPALevel(uint8_t level)
{
  ...
  setup = read_register(RF_SETUP) & (0b11111001);
  // possible values for level are 
  // RF24_PA_MIN, RF24_PA_LOW, RF24_PA_HIGH, and RF24_PA_MAX
  if (level == RF24_PA_MIN) // not necessary
    setup |= (0b00000000)
...
void RF24::setCRCLength(rf24_crclength_e length)
{
  ...
  config = read_register(CONFIG) & (0b11110011);
	//set the EN_CRC and CRC0 bits in the CONFIG register
	// possible choices for length are RF24_CRC_DISABLED, RF24_CRC_8, and RF24_CRC_16  
  if (length == RF24_CRC_DISABLED) // CRC0 set to 0
  {
    config |= 0b00001000;
    if (length == RF24_CRC16) // two bytes, CRC0 set to 1
      config |= 0b00000100;
  }
	// remember to write the register at the end
...
{
 // TODO: START HERE
  uint8_t temp = delay & (0b11110000) | count & (0b00001111);
 // Write the delay and count variables to the SETUP_RETR register
  write_register(SETUP_RETR, temp);
 //TODO: END HERE
}
